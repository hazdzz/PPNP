...
def get_parameters():
    parser = argparse.ArgumentParser(description='PPNP')
    ...
    parser.add_argument('--model', type=str, default='ppnp', help='graph neural network model')
    ...
    model_save_path = model_name + '_' + gso_type + '_' + str(alpha) + '.pth'
    model_save_path = os.path.join(model_save_dir, model_save_path)

    return device, dataset, model_name, gso_type, lr, weight_decay, droprate, n_hid, enable_bias, alpha, K, epochs, opt, early_stopping_patience, model_save_path
    
def process_data(device, dataset, gso_type, alpha):
    ...
    if device == torch.device('cpu'):
        filter = utility.calc_ppr_filter(gso, alpha, device)
...
if __name__ == "__main__":
    device, dataset, model_name, gso_type, lr, weight_decay, droprate, n_hid, enable_bias, alpha, K, epochs, opt, early_stopping_patience, model_save_path = get_parameters()
    feature, filter, label, idx_train, idx_val, idx_test, n_feat, n_class = process_data(device, dataset, gso_type, alpha)
